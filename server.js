...
app.get('/api/news/all', function(req, res, next) {
	Article.model.find().exec().then(articles => {
		if (articles) {
			res.json(articles);
		}
	});
...
app.get('/api/search/:type/:term', function(req, res, next) {
	...
	Post.model.find({
		type: type,
		slug: new RegExp(req.params['term'], 'i')
	}).exec().then(posts => {
		res.json(posts);
	});
});
...
app.get('/api/news/:slug', function(req, res, next) {
	Article.model.findOne({slug: req.params['slug']}).exec().then(article => {
		console.log(article);
		res.status(200);
	})
})
...
app.get('/api/post/:slug', function(req, res, next) {
	Post.model.findOne({slug: req.params['slug']}).populate('author').populate('accommodationType').exec().then(post => {
		if (!post) {
...
app.get('/api/accounts/:id', function(req, res, next) {
	Account.model.findOne({_id: req.params['id']}).exec().then(account => {
		if (!account) {
...
app.post('/api/posts/new', function(req, res, next) {
	...
	newPost.type = req.body.type == 'sales'
		? 'Bán'
		: 'Cho thuê';
	...
	fs.copy(image.path, './public/img/' + image.name, (err) => {
		if (err) {
			throw err;
		}
	})

	Account.model.find({email: req.body.account}).exec().then((account => {
		newPost.author = account[0];
		AccommodationType.model.find({_id: req.body.accommodation}).exec().then(accommodation => {
...
app.get('/api/accommodation_types', function(req, res, next) {
	AccommodationType.model.find().exec().then((accommodationTypes) => {
...
app.get('/api/posts', function(req, res, next) {
	Post.model.find().exec().then(function(posts) {
		res.json(posts);
	}, function(err) {
		return next(err);
	});
...
app.post('/api/accounts/validate', function(req, res, next) {
	Account.model.find({email: req.body.email}).exec().then(accounts => {
		res.json(accounts);
	}, err => {
		return next(err);
	});
...
app.post('/api/accounts/register', function(req, res, next) {
	...
	newAccount.save(err => {
		...
		res.json({account: getCleanUser(newAccount), token: generateToken(newAccount)});
	});
});

app.post('/api/login', function(req, res, next) {
	Account.model.findOne({email: req.body.email}).exec((err, account) => {
		if (err)
		...
		bcrypt.compare(req.body.password, account.password, (err, valid) => {
			if (!valid) {
				return res.status(404).json({error: true, message: 'Mật khẩu không chính xác'});
			}
			let token = generateToken(account);
			res.json({account: getCleanUser(account), token: token});
		});
	});
});
...
const getCleanUser = (user) => {
	return {_id: user._id, name: user.name, email: user.email, phone: user.phone, address: user.address}
}
